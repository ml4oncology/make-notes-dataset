...
# and `pip install make_notes_dataset` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "make_notes_dataset", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n make_notes_dataset -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "make_notes_dataset", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "make_notes_dataset", "YOUR_CONSOLE_SCRIPT"]
