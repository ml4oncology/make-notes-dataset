...
def mergeMissingNotes(dataDir, saveDir, filePartMin, filePartMax):
    """
    ...
    for ctr in range(filePartMin, filePartMax + 1):
        # load dataframe
        dfTemp = pd.read_csv( f"{dataDir}/processedMissingClinicalNotes_{ctr}.csv", index_col = 0 )
    ...
    mergedNotes.to_parquet(f'{saveDir}/merged_processed_missingClinicalNotes.parquet.gzip', compression='gzip', index=False)
...
if __name__ == "__main__":
    ...
    mergeMissingNotes( args.dataDir, args.saveDir, args.filePartMin, args.filePartMax )
